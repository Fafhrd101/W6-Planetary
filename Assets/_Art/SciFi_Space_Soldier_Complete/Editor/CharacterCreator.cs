...
public class CharacterCreator : ScriptableWizard
{
    ...
    void CreateNPC()
    {
        //Create new character and parent it
        GameObject characterGO = Instantiate(CharacterMesh, Vector3.zero, Quaternion.identity) as GameObject;
        characterGO.name = "NPC_" + CharacterMesh.name;
        characterGO.tag = "NPC";
        characterGO.layer = LayerMask.NameToLayer("Character");
        ...
        //AddComponents
        PrNPCAI newNPCAI = characterGO.AddComponent<PrNPCAI>();
        AudioSource newAudio = characterGO.AddComponent<AudioSource>();

        //Copy Components
        CopyComponents(newReferencePlayer.GetComponent<PrNPCAI>(), newNPCAI);
        CopyComponents(newReferencePlayer.GetComponent<Rigidbody>(), characterGO);
        CopyComponents(newReferencePlayer.GetComponent<AudioSource>(), newAudio);
        ...
        if (WeaponGrip != null && RefWeaponGrip != null)
        {
            ...
            newNPCAI.WeaponGrip = WeaponGrip.transform;
        ...
        //Set Sensor Position
        GameObject NPCSensors = new GameObject("NPCSensor") as GameObject;
        NPCSensors.transform.parent = characterGO.transform;
        NPCSensors.transform.position = new Vector3(0, 1.7f, 0);
        newNPCAI.eyesAndEarTransform = NPCSensors.transform;

        //Set New Skinned Meshes to inventory
        characterGO.GetComponent<PrNPCAI>().MeshRenderers = characterGO.GetComponentsInChildren<SkinnedMeshRenderer>();
