...
public class PrTopDownCamera : MonoBehaviour {
    ...
    private Vector3 _randomShakePos = Vector3.zero;
    ...
    {
        _randomShakePos = new Vector3(Random.Range(-shakeFac, shakeFac), Random.Range(-shakeFac, shakeFac), Random.Range(-shakeFac, shakeFac));
        if (isExplosion)
            return _randomShakePos * (actualExpShakeTimer / shakeExpTimer);
        else
            return _randomShakePos * (actualShakeTimer / shakeTimer);
    }
