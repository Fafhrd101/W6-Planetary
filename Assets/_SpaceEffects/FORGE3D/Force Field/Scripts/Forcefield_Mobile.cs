...
namespace Forge3D
{
    ...
    {
        // Holder for who owns this FF
        public ShipEquipment energySource;
        ...
        {
            var projectile = collisionInfo.transform.GetComponent<Projectile>();
            //if (projectile != null)
             //   return;
            //else print("projectile hit");
            var damage = projectile != null ? projectile.damage : 10; // Should be checking beam + missile here
            //print("Collision damage "+collisionInfo.transform.name+damage);
            if (CollisionEnter)
                foreach (ContactPoint contact in collisionInfo.contacts)
                    OnHit(contact.point, damage);
        }

        // MASK MANAGEMENT
        // Use this method to pass new impact points from any other script
        public void OnHit(Vector3 hitPoint, float damage, float hitAlpha = 1.0f)
        {
            if (energySource != null)
            {

                // Shield check
                if (energySource.energyAvailable < 25)
                {
                    //print("Shield down!");
                    energySource.shieldActive = false;
                    return;
                }
                // in case no one else thinks to turn it back on
                else if (energySource.energyAvailable >= 25)
                    energySource.shieldActive = true;
                
                energySource.energyAvailable -= damage / 2;
            }
            ...
            if (curTime >= ReactSpeed)
            {
                ...
                if (FixNonUniformScale)
                {
                    if (!Mathf.Approximately(transform.lossyScale.x, transform.lossyScale.y) || !Mathf.Approximately(transform.lossyScale.y, transform.lossyScale.z) || !Mathf.Approximately(transform.lossyScale.y, transform.lossyScale.z))
                    {
                        ...
                        shaderProps[curProp].x *= lossyScale.x;
                        shaderProps[curProp].y *= lossyScale.y;
                        shaderProps[curProp].z *= lossyScale.z;
                    }
                }
                ...
                //print("OnHit() ForceField.cs "+gameObject.name);
            }
        }
