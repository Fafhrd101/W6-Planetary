    ...
    public class F3DBeam : MonoBehaviour
    {
        ...
        { 
            ...
            if (Physics.Raycast(ray, out hitPoint, MaxBeamLength, layerMask))
            {
                ...
                if (hitPoint.transform.tag == "Ship")
                    hitPoint.transform.GetComponent<Ship>().TakeDamage(20 * Time.deltaTime, PlayerShot);
                else if (hitPoint.transform.tag == "Asteroid")
                    hitPoint.transform.GetComponent<Asteroid>().TakeDamage(20 * Time.deltaTime);
                else if (hitPoint.transform.tag == "StationParts"||hitPoint.transform.tag == "Station")
                    hitPoint.transform.GetComponentInParent<Station>().TakeDamage(/*DPS*/20 * Time.deltaTime, Ship.PlayerShip, PlayerShot); // only players have, so assume attacker
