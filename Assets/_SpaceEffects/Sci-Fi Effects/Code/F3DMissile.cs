...
namespace FORGE3D
{
    public class F3DMissile : MonoBehaviour
    {
        ...
        [Tooltip("Speed which it can make flight adjustments")]
        public float alignSpeed = 1f;
        [Tooltip("Only applicable with unguided")]
        ...
        private void OnHit()
        {
            ...
            isHit = true;
            //TakeDamage(float damage, Ship owner, bool isPlayerShot, bool shouldShake=true)
        ...
        private void Update()
        {
            ...
            if (isHit)
            {
                ...
                if (!isFXSpawned)
                {
                    ...
                    if (target != null)
                    {
                        // print("Missile hit "+target.name+" tagged "+target.tag);
                        // print("Firedby "+firedBy);

                        if (target.gameObject.GetComponent<Ship>())
                            target.gameObject.GetComponent<Ship>().TakeDamage(500, playerShot);
                        else if (target.gameObject.GetComponent<Asteroid>())
                            target.gameObject.GetComponent<Asteroid>().TakeDamage(100);
                        else if (target.gameObject.GetComponentInParent<Station>())
                            target.gameObject.GetComponentInParent<Station>().TakeDamage(500, null, playerShot);
                    }
            ...
            else
            {
                ...
                else
                {
                    ...
                    if (timer >= lifeTime)
                    {
                        //print("removing missile outside of it's range");
                        // Do not detonate
                        isFXSpawned = true;
                        OnHit();
                    }
