...
public class CameraController : MonoBehaviour
{
    ...
    private void Update()
    {
        ...
        {
            transform.position = Ship.PlayerShip.cockpit.position+cockpitOffset;
        }
        ...
        if (Input.GetKeyDown(KeyCode.V))
        {
            if (PlayArcadeIntegration.Instance.devBuild)
            {
                ...
                {
                    State = CameraState.Orbit;
                    Ship.PlayerShip.cockpit.gameObject.SetActive(false);
                    Ship.PlayerShip.bodyVisuals.enabled = true;
                }
                else if (State == CameraState.Chase)
                {
                    State = CameraState.Cockpit;
                    Ship.PlayerShip.cockpit.gameObject.SetActive(true);
                    Ship.PlayerShip.bodyVisuals.enabled = false;
                }
                else // orbit
                {
                    State = CameraState.Chase;
            ...
            // version without Cockpit
             else State = State == CameraState.Chase ? CameraState.Orbit : CameraState.Chase;
        }
    ...
    public void ToggleFlightMode()
    {
        ...
        else
        {
            ...
            if (Ship.PlayerShip.UsingMouseInput) Cursor.visible = false;
            ControlStatusUI.Instance.SetControlStatusText(Ship.PlayerShip.UsingMouseInput ? "Mouse Flight" : "Keyboard Mode");
        }
    }
    ...
    void FixedUpdate()
    {
        ...
        if (State == CameraState.Orbit)
        {
            ...
            Vector3 position = rotation * _orbitOffset + target.position + target.up * Distance * 0.1f;
            ...
            if (State == CameraState.Cockpit)
            {
                if (Ship.PlayerShip.cockpit != null)
                {
                    ...
                    else
                    {
                        ...
                        float verticalOffset = Mathf.Lerp(0, 30,
                ...
                } else print(Ship.PlayerShip+" is munged!");
            }
        }
