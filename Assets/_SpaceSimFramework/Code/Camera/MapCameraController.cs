...
public class MapCameraController : MonoBehaviour
{
    ...
    void Update()
    {
        ...
        // else if (CanvasViewController.IsMapActive)
        // {
        //     if (Ship.PlayerShip == null)
        //         return;
        //     Vector3 targetPosition = Ship.PlayerShip.transform.position;
        //     targetPosition.y = 0;
        //     transform.position = targetPosition + tacticalCameraOffset;
        //     return;   
        // }
...
#endregion
        #region SubFunctionInput
        if (Input.GetKey(KeyCode.S) || Input.mousePosition.y >= Screen.height - ScreenEdgeBorderThickness)
        {
            _panMovement += forwardVector * _panSpeed * Time.deltaTime;
        }
        if (Input.GetKey(KeyCode.W) || Input.mousePosition.y <= ScreenEdgeBorderThickness)
        {
            _panMovement -= forwardVector * _panSpeed * Time.deltaTime;
        }
