...
public class MissionControl : MonoBehaviour {
    ...
    {
        Mission job = null;
        int mission_i = Random.Range(0, 3);
        if (mission_i == 0)
        {
    ...
    public static void RegisterKill(Station kill)
    {
        if (CurrentJob == null)
            return;
        ...
        {
            // Friendly ship killed, abort
            ConsoleOutput.PostMessage("Mission failed! You have destroyed a friendly station!", Color.blue);
            CurrentJob = null;
            MissionUI.Instance.panel.SetActive(false);
        }
        else
