...
public class NavigationContactList : MonoBehaviour {
    ...
    private void Awake()
    {
        HeaderText.text = "Current Sector:\n" + SectorNavigation.CurrentSector;
        ...
        foreach (GameObject planet in SectorNavigation.Planets)
        {
            Color relationColor = Player.Instance.playerFaction.GetTargetColor(planet);
            AddContact(planet, Color.white, (int)ObjectIcon.Planet);
        }
    }
    ...
    private bool ShipOutsideScannerRange(Vector3 shipPosition)
    {
        foreach (var playerShip in Player.Instance.Ships)
        {
            ...
            if (Vector3.Distance(playerShip.transform.position, shipPosition) < playerShip.GetComponent<Ship>().shipModelInfo.ScannerRange)
                return false;
        }
