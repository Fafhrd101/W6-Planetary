            ...
            case PART2_DESTROYSHIPS:
                if (currentMenu == null)
                {
                    if (!waitingForInput)
                    {
                        ...
                        Targets[1].GetComponent<ShipAI>().PatrolPath(SectorNavigation.GetPatrolWaypoints());
                        Targets[2].GetComponent<ShipAI>().PatrolPath(SectorNavigation.GetPatrolWaypoints());
    ...
    public void ReportCheckPointAchieved()
    {
        ...
        switch (checkpoint)
        {
            ...
            case PART2_WELCOME:
                ...
                // Stop the ship
                Ship.PlayerShip.PlayerInput.throttle = 0;   
                break;
            case PART2_SELECTTARGET:
