...
public class Targeting : MonoBehaviour {
    ...
    {
        //calculate intercept
        var interceptPoint = PredictTargetLead3D(shooter, target, projectileSpeed);

        if (Camera.main is not null)
        {
            var screenPos = Camera.main.WorldToScreenPoint(interceptPoint);
    ...
    public static Vector3 SlightOffset(Vector3 pos, float missOffset = 3.0f)
    {
        return new Vector3(pos.x + UnityEngine.Random.Range(-missOffset, missOffset),
            pos.y + UnityEngine.Random.Range(-missOffset, missOffset),
            pos.z + UnityEngine.Random.Range(-missOffset, missOffset));
    }
}
